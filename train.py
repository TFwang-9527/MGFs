...
from utils.loss_utils import l1_loss, ssim,ssim_mask,l1_loss_mask,ssim_mask2,edge_mask_loss,ms_multi_mask,multi_mask
...
from utils.image_utils import psnr,psnr_mask,psnr_mask2,calculate_lpips
...
def L1_loss_appearance_mask(image, gt_image, mask, edge_mask, gaussians, view_idx, return_transformed_image=False):
    appearance_embedding = gaussians.get_apperance_embedding(view_idx)
    # center crop the image
    origH, origW = image.shape[1:]
    H = origH // 32 * 32
    ...
    crop_edge_mask = (edge_mask.unsqueeze(0))[:, top:top + H, left:left + W]
    # down sample the image
    crop_image_down = \
    torch.nn.functional.interpolate(crop_image[None], size=(H // 32, W // 32), mode="bilinear", align_corners=True)[0]
    ...
    if not return_transformed_image:
        return l1_loss_mask(transformed_image, crop_gt_image, crop_mask)
    else:
        transformed_image = \
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if (iteration > 7000):
            viewpoint_cam.mask_image = viewpoint_cam.white_mask_image

        render_pkg = render(iteration, viewpoint_cam, gaussians, pipe, background, kernel_size=dataset.kernel_size)
        ...
        RGB_weight = viewpoint_cam.RGB_weight.to(image.device)
        edge_mask = viewpoint_cam.edge_mask.to(image.device)
        mask_tensor = viewpoint_cam.mask_image.unsqueeze(0)
        multi_mask_tensor = viewpoint_cam.multi_mask_image.unsqueeze(0)
        gt_image[:, mask_tensor.squeeze(0) == 0] = 0
        ##############################################################################
        if dataset.use_decoupled_appearance:
            Ll1 = L1_loss_appearance_mask(image, gt_image, mask_tensor , edge_mask, gaussians, viewpoint_cam.idx)

        rgb_loss =(1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * ( 1.0 - ssim_mask(image, gt_image, mask_tensor))
        distortion_map = rendering[8, :, :]


        distortion_map = get_edge_aware_distortion_map(gt_image, distortion_map)
        distortion_loss= distortion_map.sum()/mask_tensor.sum()
        ...
        depth_normal_loss = (RGB_weight*normal_error).sum()/mask_tensor.sum()
        ...
        if is_save_images and (iteration % opt.densification_interval == 0):
            with torch.no_grad():
                ...
                #eval_cam = allCameras[10]

                rendering = render(iteration,eval_cam, gaussians, pipe, background, kernel_size=dataset.kernel_size)["render"]
                image = rendering[:3, :, :]
            ...
            torchvision.utils.save_image(image_to_show, f"{dataset.model_path}/log_images/"+"%05d" % iteration + "_" + viewpoint_cam.image_name + ".jpg")
        ...
        with torch.no_grad():
            ...
            if (iteration in saving_iterations):
                tqdm.write("[ITER {}] Saving Gaussians".format(iteration))  # 使用 tqdm.write
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    rendering = renderFunc(iteration, viewpoint, scene.gaussians, *renderArgs)["render"]
                    image = rendering[:3, :, :]
                    normal = rendering[3:6, :, :]
                    ...
                    #############################################
                    mask_tensor = viewpoint.mask_image.unsqueeze(0)
                    l1_test_mask += l1_loss_mask(image, gt_image, mask_tensor).mean().double()
                    ssim_test_mask += ssim_mask(image, gt_image, mask_tensor).mean().double()
                    lpips_test+= calculate_lpips(image, gt_image, mask_tensor)
                    #########################################################
                ssim_test /= len(config['cameras'])
                ssim_test_mask /= len(config['cameras'])
                lpips_test /= len(config['cameras'])
                psnr_test /= len(config['cameras'])
                l1_test /= len(config['cameras'])
                psnr_test_mask /= len(config['cameras'])
                l1_test_mask /= len(config['cameras'])
                print("\n[ITER {}] Evaluating {}: LPIPS {} ".format(iteration, config['name'], lpips_test))
                print("\n[ITER {}] Evaluating {}: L1 {} L1_mask {}".format(iteration, config['name'], l1_test, l1_test_mask))
                print("\n[ITER {}] Evaluating {}: PSNR {} PSNR_mask {} ".format(iteration, config['name'], psnr_test, psnr_test_mask))
                if tb_writer:
